...
MATH_INLINE float Float4Sum(float4_simd float4) {
  ...
  float4_simd shuf = SIMD_SHUFFLE(float4, 2, 3, 0, 1);
...
MATH_INLINE void QuatToMat4(const quat q, mat4* pDst) {
  ...
  const float3_simd xxw = SIMD_SHUFFLE(q.simd, VEC_X, VEC_X, VEC_W, SIMD_NIL);
  const float3_simd xyx = SIMD_SHUFFLE(q.simd, VEC_X, VEC_Y, VEC_X, SIMD_NIL);
  const float3_simd xx_xy_wx = s * xxw * xyx;

  const float3_simd yyw = SIMD_SHUFFLE(q.simd, VEC_Y, VEC_Y, VEC_W, SIMD_NIL);
  const float3_simd yzy = SIMD_SHUFFLE(q.simd, VEC_Y, VEC_Z, VEC_Y, SIMD_NIL);
  const float3_simd yy_yz_wy = s * yyw * yzy;

  const float3_simd zxw = SIMD_SHUFFLE(q.simd, VEC_Z, VEC_X, VEC_W, SIMD_NIL);
  const float3_simd zzz = SIMD_SHUFFLE(q.simd, VEC_Z, VEC_Z, VEC_Z, SIMD_NIL);
...
MATH_INLINE mat4 Mat4Mul(const mat4 l, const mat4 r) {
  return (mat4){.simd = SIMD_SHUFFLE(l.simd,
