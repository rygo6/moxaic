...
static uint32_t FindQueueIndex(
  ...
  printf("Finding queue family: graphics=%s compute=%s transfer=%s globalPriority=%s present=%s... ",
  ...
  for (uint32_t i = 0; i < queueFamilyCount; ++i) {
    const char graphicsSupport = queueFamilyProperties[i].queueFamilyProperties.queueFlags & VK_QUEUE_GRAPHICS_BIT;
    const char computeSupport = queueFamilyProperties[i].queueFamilyProperties.queueFlags & VK_QUEUE_COMPUTE_BIT;
    const char transferSupport = queueFamilyProperties[i].queueFamilyProperties.queueFlags & VK_QUEUE_TRANSFER_BIT;
    const char globalPrioritySupport = queueFamilyGlobalPriorityProperties[i].priorityCount > 0;
...
int MxRendererInit() {
  ...
  // Device
  {
    ...
    const char* ppEnabledDeviceExtensionNames[] = {
        ...
        g_ExternalMemoryExntensionName,
        g_ExternalSemaphoreExntensionName,
        g_ExternalFenceExntensionName,
    };
    VkDeviceQueueGlobalPriorityCreateInfoEXT deviceQueueGlobalPriorityCreateInfo = {
        .sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_GLOBAL_PRIORITY_CREATE_INFO_EXT,
        .globalPriority = g_IsCompositor ? VK_QUEUE_GLOBAL_PRIORITY_MEDIUM_EXT : VK_QUEUE_GLOBAL_PRIORITY_LOW_EXT,
    };
    float                   pQueuePriorities[] = {g_IsCompositor ? 1.0f : 0.0f};
